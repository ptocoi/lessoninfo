<div class="comWrap">
	<h2>
		...
		<div class="titleBtn layerBtn bg_color1  clearfix positionA" style="top:0; right:0;">
			<a>글쓰기<img alt="" src="../images/btn/btn_rightArrow3.png"></a>	
		</div>
		<?php } ?>
	</h2>
	...
	<?php 
		...
		preg_match("/<img[^>]*src=[\"']?([^>\"']+)[\"']?[^>]*>/i", $list['wr_content'], $_img);
		...
		if(!$_img){	 // 내용상 이미지가 없다면
			...
			if($get_file[0]['view']){
				...
				$thumb_file_list = $thumb_path . "/169x100_".$new_imgname."_".$get_file['wr_no'];
				if(!file_exists($thumb_file_list)){
					...
					if(!$gdversion){ 
						...
						if($get_img[0] <= $re_img_width){
							...
							// 섬네일 파일 체크
							if(!file_exists($thumb_file_list)){
								$utility->createThumb_list(169,100,$src, $thumb_file_list); // list 페이지 썸네일
							}
						}
					}
				}
			...
			if($_img[0]){
				$_imgs = explode('src=\"',$_img[0]);
				$_imgs = explode('\" />',$_imgs[1]);
				$img = $_imgs[0];
			} else {
				$img = "../images/basic/img_01.gif";	// 169 x 100
			}
		}
	?>
		<li class="webzineList positionR">
			<dl class="clearfix">
				<div style="position:absolute; left:0;top:0;">
					<a href="../community/view.html?bo_table=<?php echo $bo_table;?>&wr_no=<?php echo $list['wr_no'];?>"><img src="<?php echo $img;?>"></a>
				</div>
				<dt> 
					<a href="../community/view.html?bo_table=<?php echo $bo_table;?>&wr_no=<?php echo $list['wr_no'];?>">
