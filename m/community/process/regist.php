<?php
	...
	switch($mode){
		...
		if($mode=='insert' || $mode=='reply' || $mode=='update'){	// 글 작성/답변/수정 완료
			if($result){
				$utility->popup_msg_js("", "../list.php?bo_table=".$bo_table);
			}
			/*
			if($alice['https_url'])
		...
		case 'comment_insert':
			...
			if($result){	 // 작성 성공
				...
				$utility->popup_msg_js("","../view.html?board_code=".$board_code."&code=".$code."&bo_table=".$bo_table."&wr_no=".$wr_no."&cwin=".$cwin."#c_".$comment_id);
			} else {
