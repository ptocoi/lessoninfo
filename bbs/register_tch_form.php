...
if ($w == "") {
    ...
    */
    ...
    $member['mb_1']           = get_text($member['mb_1']); // 근무희망지역
    $member['mb_2']           = get_text($member['mb_2']); // 최종 학력 
    $member['mb_3']           = get_text($member['mb_3']); // 경력
    $member['mb_4']           = get_text($member['mb_4']); // 근무 형태 
    $member['mb_5']           = get_text($member['mb_5']); // 희망급여
    $member['mb_6']           = get_text($member['mb_6']); // 교습가능 과목
    $member['mb_7']           = get_text($member['mb_7']); // 성별 
    $member['mb_8']           = get_text($member['mb_8']); // 나이
...
if($type==='lesson'){
	include_once($member_skin_path.'/register_tch_form_lesson.skin.php');
}else if($type=='instructor'){
	include_once($member_skin_path.'/register_tch_form.skin.php');
}else{
	alert('w 값이 제대로 넘어오지 않았습니다.');
}
