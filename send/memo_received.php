<?php
		/*
		* /application/send/memo_received.php
		* @author Harimao
		* @since 2015/03/16
		...
		*/
		...
		// paging
		$total_page = $send_memo['total_page'];
		$pages = $utility->user_get_paging($page_rows, $page, $total_page,"./memo_received.php?page_rows=".$page_rows."&page=");
		...
		##
		# Include view
		if(is_file($alice['self'] . 'views/memo_received.html'))
			include_once $alice['self'] . 'views/memo_received.html';
		else
			$config->error_info( $alice['self'] . 'views/memo_received.html' );
