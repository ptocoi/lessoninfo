<?php
		/*
		* /application/company/process/custom.php
		* @author Harimao
		* @since 2013/10/14
		* @last update 2015/03/11
		* @Module v3.5 ( Alice )
		* @Brief :: Customized process
		* @Comment :: 맞춤 인재정보 프로세스
		*/
		...
		switch($mode){
			...
			case 'custom_update':
				...
				$result = $alba_company_control->custom_updates($vals,$sel_custom);

				if($result){
					$utility->popup_msg_js("","../alba_customized.php");
				} else {
					$utility->popup_msg_js($alba_company_control->_errors('0006'));
				}
