<?php
		/*
		* /application/alba/alba_list_today.php
		* @author Harimao
		* @since 2013/08/12
		...
		*/
		...
		//  and `wr_report` = 0
		$list_con .= " where `wr_open` = 1 and `wr_is_adult` = 0 and `wr_wdate` > curdate() and `is_delete` = 0 ";
		$list_list = $alba_user_control->__AlbaList( $page, $page_rows, $list_con );
		$total_page = $list_list['total_page'];
		$pages = $utility->user_get_paging($page_rows, $page, $total_page,"./alba_list_today.php?page_rows=".$page_rows."&".$list_list['send_url']."&page=");

		##
		# Include view
		if(is_file($alice['self'] . 'views/alba_list_today.html'))
			include_once $alice['self'] . 'views/alba_list_today.html';
		else
			$config->error_info( $alice['self'] . 'views/alba_list_today.html' );
