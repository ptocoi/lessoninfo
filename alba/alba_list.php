<?php
		/*
		* /application/alba/alba_list.php
		* @author Harimao
		* @since 2013/08/12
		...
		*/
		...
		if($service_check['is_pay']){
			$list_con .= " and ( `wr_service_basic_sub` >= curdate() ) ";
		}
		$list_list = $alba_user_control->__AlbaList( $page, 30, $list_con );
		$total_page = $list_list['total_page'];
		$pages = $utility->user_get_paging($page_rows, $page, $total_page,"./alba_list.php?page_rows=".$page_rows."&".$list_list['send_url']."&page=");

		//if($page_rows || $page) $list_url = "&page_rows=" . $page_rows . "&page=" . $page . "&sort=" . $sort;
