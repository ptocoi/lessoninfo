...
<div id="platinum" class="style2 border<?php echo $design['sub_platinum_border'];?><?php echo ($design['sub_platinum_margin'])?' margin':'';?> number<?php echo $sub_platinum_row;?> mt25 clearfix">
	...
	<ul class="clearfix positionR">
		...
		<?php 
			...
			$no = $val['no'];
			$list = $alba_user_control->get_alba_service($no,"sub_platinum");
			switch($design['sub_platinum']){
				case '01':	// 플래티넘디자인1
		?>
		...
		<?php 
		if($platinum_rest != ($sub_platinum_row - 1)){
			for($i=0;$i<$platinum_rest;$i++){ 
			$Rend = ($i == ($platinum_rest-1)) ? 'Rend' : '';
		?>
			<!-- 신규광고 신청 -->
...
var platinum_open = function( no ){	// 플래티넘 공고 상세보기
	...
	$('#platinum_quickView').load('./views/_load/alba.php', { mode:'platinum', no:no }, function(){
		//alert( position_top );
		position_tops = position_top - 2;
		$(this).css( "top", position_tops+"px" );
		$(this).show();
	});
}
