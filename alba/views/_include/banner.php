...
<div id="box" class="style2 border<?php echo $design['sub_banner_border'];?><?php echo ($design['sub_banner_margin'])?' margin':'';?> number<?php echo $sub_banner_row;?> mt25 clearfix">
	...
	<ul class="clearfix positionR">  
		...
		<?php 
			...
			$no = $val['no'];
			$list = $alba_user_control->get_alba_service($no,"sub_banner");
			switch($design['sub_banner']){
				case '01':	// 배너형 디자인1
		?>
		...
		<?php 
		if($banner_rest != ($sub_banner_row - 1)){
			for($i=0;$i<$banner_rest;$i++){ 
			$Rend = ($i == ($banner_rest-1)) ? 'Rend' : '';
		?>
...
<script>
var banner_open = function( no ){	// 배너형 공고 상세보기
	var $banner_list = $('#banner_list_'+no);
	var position_top = $banner_list.position().top;

	$('#banner_quickView').hide();
	$('#banner_quickView').load('./views/_load/alba.php', { mode:'banner', no:no }, function(){
