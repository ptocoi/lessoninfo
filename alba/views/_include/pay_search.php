	...
	<ul>
		...
		<li class="resultSet clearfix" id="devResultSet">
			<div class="resultBox clearfix resultSetEmpty">
				...
				<!-- 선택조건 있을때 display-->
				<dl class="selectBox clearfix" style="display:<?php echo ( $mode=='search' && ($category_top_val || $job_type_0 || $wr_area_0 || $wr_subway_0) )?'':'none';?>;">
					<dt class="pb5"><strong>선택된 조건(<?php if($type!='day'){ ?>급여,<?php } ?>근무지역,역세권)</strong></dt>
					<dd>
						<ul class="selectList">
						<?php
						if( $mode=='search' ){
							...
							if($search_mode_sels){
								...
								foreach($search_mode_sels as $val){
									...
									echo "<li id=\"select_".$val_exp[0]."\" class=\"category\">".$search_text."&nbsp;<input type=\"hidden\" name=\"".$search_mode."_sels[]\" value=\"".$val."\" id=\"".$search_mode."_sels_".$vals_id."\"></li>";
									// <button type=\"button\" class=\"close\" onclick=\"search_close('category','".$search_mode."','".$val_exp[0]."');\">x</button>
				...
				<p class="noSelect" style="display:<?php echo ( $mode=='search' && ($category_top_val || $job_type_0 || $wr_area_0 || $wr_subway_0) )?'none':'';?>;">선택된 조건(<?php if($type!='day'){ ?>급여,<?php } ?>근무지역,역세권)이 없습니다.</p>
