...
<div class="searchBox">
	<ul>
		<li class="workArea clearfix" style="display:block;"><!--  시도별정규직정보,대학가정규직정보에서 display:none -->
		<dl>
		...
		<dl>
			...
			<dd class="itemBoxPartSet">
				...
				<?php
				if($wr_jobtype_2 && $wr_jobtype_1[$first_jobtype]){
		...
		<li class="resultSet clearfix" id="devResultSet">
			<div class="resultBox clearfix resultSetEmpty">
						...
						if( $mode=='search' ){
							if($area_sels){	// 지역검색
								...
								foreach($area_sels as $val){
									...
									if($val_exp[2]){
										...
										$select_id = ($val_exp[1]=='all') ? 'all' : $select_class = $val_exp[0]."_".$val_exp[1];
									} else if($val_exp[0]){
										$select_id = $val_exp[0];
										$select_class = "";
									}
									
									$select_add_class = $val_exp[0];

									echo "<li id=\"select_".$select_id."\" class=\"select_".$select_class." select_".$select_add_class." area\">".$area_text."&nbsp;<button type=\"button\" class=\"close\" onclick=\"search_close('area','".$val_exp[0]."','".$val_exp[1]."','".$val_exp[2]."');\">x</button><input type=\"hidden\" name=\"area_sels[]\" value=\"".$val."\" id=\"area_sels_".$vals_id."\"></li>";
							...
							if($subway_sels){	// 역세권 검색
								...
								foreach($subway_sels as $val){
									...
									if($val_exp[2]){
										...
										$select_id = $select_class = $val_exp[0]."_".$val_exp[1];
									} else if($val_exp[0]){
										$select_id = $val_exp[0];
										$select_class = "";
									}
									...
									echo "<li id=\"select_".$select_id."\" class=\"select_".$select_class." select_".$select_add_class." subways\">".$subway_text."&nbsp;<button type=\"button\" class=\"close\" onclick=\"search_close('subway','".$val_exp[0]."','".$val_exp[1]."','".$val_exp[2]."');\">x</button><input type=\"hidden\" name=\"subway_sels[]\" value=\"".$val."\" id=\"subway_sels_".$vals_id."\"></li>";
							...
							if($jobtype_sels){	// 직종 검색
								...
								foreach($jobtype_sels as $val){
									...
									echo "<li id=\"select_".$select_id."\" class=\"select_".$select_class." select_".$select_add_class." jobtypes\">".$jobtype_text."&nbsp;<button type=\"button\" class=\"close\" onclick=\"search_close('jobtype','".$val_exp[0]."','".$val_exp[1]."','".$val_exp[2]."');\">x</button><input type=\"hidden\" name=\"jobtype_sels[]\" value=\"".$val."\" id=\"jobtype_sels_".$vals_id."\"></li>";
				...
				<p class="noSelect" style="display:<?php echo ( $mode=='search' && ($wr_area_0 || $wr_subway_0 || $wr_jobtype_0) )?'none':'';?>;">선택된 조건(근무지역,역세권,업직종)이 없습니다.</p>
