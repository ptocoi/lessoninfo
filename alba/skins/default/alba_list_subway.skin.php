...
<script type="text/javascript" src="<?php echo $alice['alba_path'];?>/skins/default/alba_list_subway.skin.js"></script>
...
<section id="content" class="content_wrap clearfix">
	...
	<div class="content2_wrap clearfix"> 
		...
		<div class="NowLocation mt35 clearfix">
			<p> <a href="/">HOME</a> > <a href="<?php echo $alice['alba_path'];?>">채용정보</a> > <a href="<?php echo $alice['alba_path'];?>/alba_list_area.php">지역별 채용정보</a> > <strong>역세권 채용정보</strong> </p>
		</div>
		<?php /* //navigation */ ?>
		...
		<div class="listWrap mt20">
			<h2 class="skip"><img src="../images/tit/job_tit_08.gif"  alt="역세권 정규직 정보"></h2>
			<div class="jobContentWrap positionR mt10 clearfix">
				<div class="subwayWrap  clearfix">
					<ul class="tabMenu clearfix">
						...
						if($category_subway){
							$i = 0;
							foreach($category_subway as $key => $val){
					...
					<div id="category_second">
						...
						<?php 
						if($mode=='search' && $alba_subway_1){ // 검색일때 
							$checked_arr = array();
							foreach($alba_subway_1 as $type_key => $type_val){
							$pcode_list = $category_control->category_pcodeList('subway',$type_key,"", " and `view` = 'yes' ");
						...
						?>
								...
								$sub_list = $category_control->category_pcodeList('subway',$val,"", " and `view` = 'yes' ");
								$display = (@in_array($val,$checked_arr)) ? 'block' : 'none';
								?>
								<div class="mt5 smallSubway border_color1 bg_color2 clearfix category_sub category_sub_<?php echo $val;?>" id="category_sub_<?php echo $val;?>" style="display:<?php echo $display;?>;">
									<ul class="listArea">
									<?php
						...
						<?php } else { // 검색이 아닐때 ?>
						<div class="listLocal clearfix category_middle" id="category_middle_<?php echo $category_subway[0]['code'];?>" style="display:;">
							<div class="mt5 middleSubway clearfix">
								<ul class="listArea">
								<?php
								$pcode_list = $category_control->category_pcodeList('subway',$category_subway[0]['code'],"", " and `view` = 'yes' ");
								foreach($pcode_list as $val){
								$name = $utility->remove_quoted($val['name']);	 // (쌍)따옴표 등록시 필터링
								$category_count = $alba_user_control->get_category_count($val['code'],'wr_subway_line_0');
								?>
								<li id="category_middle_list_<?php echo $val['code'];?>">
									<span>
										<input type="checkbox" name="<?php echo $search_mode;?>_1[<?php echo $val['p_code'];?>][]" id="<?php echo $search_mode;?>_1_<?php echo $val['code'];?>" class="<?php echo $search_mode;?>_1_<?php echo $category_subway[0]['code'];?>" value="<?php echo $val['code'];?>" onclick="category_middles(this,'<?php echo $category_subway[0]['code'];?>');"/>
										<label for="<?php echo $search_mode;?>_1_<?php echo $val['code'];?>"><?php echo $name;?></label>
									</span>
									<span class="sfont text_color1"> (<?php echo number_format($category_count);?>)</span>
								</li>
				...
				<?php include_once "./views/_include/subway_search.php";?>
			</div>
		</div>
		</form>
		<!--  // 역세권 정규직 검색영역 -->
		...
		<div id="JobListForm" class="positionR mt30 clearfix">
			...
			<div class="ListFunc positionA" style="top:-5px; right:0;">
				<span class="choiceLink pr5">
					<span class="Link1"><a href="./alba_list_subway.php?<?php echo $list_list['send_url'];?>&view_type=list#result"><img width="25" height="19" alt="" src="../images/icon/icon_choiceLink1_<?php echo (!$view_type||$view_type=='list')?'on':'off';?>.gif" class="vb" id="choiceLink_list"></a></span>
					<span class="Link2"><a href="./alba_list_subway.php?<?php echo $list_list['send_url'];?>&view_type=gallery#result"><img width="25" height="19" alt="" src="../images/icon/icon_choiceLink2_<?php echo ($view_type=='gallery')?'on':'off';?>.gif" class="vb" id="choiceLink_gallery"></a></span>
				</span>
				<span>
					<select title="pagesize" name="sort" style="width:100px;" class="ipSelect2" onchange="list_sorting(this,'alba_list_subway');">
