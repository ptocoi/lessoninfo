$(function(){
	...
	$('#alba_search_keyword').keydown(function(event){	// 검색어 필드 엔터 이벤트
		if(event.keyCode==13){	
			searchFrm_submit();
			return false;
		}
...
var area_sub_all = function( vals, first_code, second_code, first_text, second_text ){	
	...
	if(checked){
		...
		$('.area_sel_'+first_code+"_"+second_code).remove();
...
/*
var wr_area1_alls = function( vals, first_area ){	// 2차 지역 전체 선택
	var sels = vals.value;
	var checked = vals.checked;
	if(checked==true){
		$('.'+first_area).attr('checked',false);
		$('.'+first_area).attr('disabled',true);
		$('.area_third_'+first_area).remove();
	} else {
		$('.'+first_area).attr('disabled',false);
	}
}
*/
