...
<section id="content" class="content_wrap clearfix">
	...
	<div class="content2_wrap clearfix"> 
		...
		<div class="NowLocation mt35 clearfix">
			<p> <a href="/">HOME</a> > <a href="<?php echo $alice['alba_path'];?>">채용정보</a> > <a href="<?php echo $alice['alba_path'];?>/alba_list_pay.php">급여별 채용정보</a> > <strong><?php echo ($type=='day')?'지원별':'급여별';?> 채용정보</strong> </p>
		</div>
		<?php /* //navigation */ ?>
		...
		<div class="listWrap mt20">
			<?php if($type=='day'){ // 조건별 정규직 정보 ?>
			<h2 class="skip">지원별 정규직 정보</h2>
			<div class="jobContentWrap positionR mt10 clearfix">
				<div class="resumeContentWrap positionR clearfix">
					<div class="targetWrap positionR">
						<?php 
							foreach($alba_pay_type_list as $val){
							$name = $utility->remove_quoted($val['name']);	 // (쌍)따옴표 등록시 필터링
							$checked = ($val['code']==$wr_pay_support) ? 'checked' : '';
						?>
						<input type="radio" name="wr_pay_support" id="wr_pay_support_<?php echo $val['code'];?>" value="<?php echo $val['code'];?>" class="chk" <?php echo $checked;?>>
						<label for="wr_pay_support_<?php echo $val['code'];?>"><?php echo $name;?></label>
						<?php } // alba_pay_type_list foreach end. ?>
					</div>
				<?php include_once "./views/_include/pay_search.php";?>
				</div>
			</div>
			<?php } else { ?>
			<h2 class="skip">급여별 정규직 정보</h2>
			<div class="jobContentWrap positionR mt10 clearfix">
				<div class="payWrap clearfix">
					<ul class="tabMenu clearfix">
						...
						if($alba_pay_list){
							$i = 0;
							foreach($alba_pay_list as $key => $val){ 
							$name = $utility->remove_quoted($val['name']);	 // (쌍)따옴표 등록시 필터링
							$checked = (@in_array($val['code'],$category_top_val)) ? 'check' : '';
							if($mode=='search'){
								if($category_top_val[0]){
					...
					if($alba_pay_list) {
						...
						$pcode_list = $category_control->category_pcodeList('alba_pay',$val['code']);
						if($mode=='search'){
							if($category_top_val[0]){
								$display = ($search_sel_1[$val['code']] && $val['code'] == $category_top_val[0]) ? "block" : "none";
							} else {
					...
					?>
					<div class="listPay clearfix" id="category_middle_<?php echo $val['code'];?>" style="display:<?php echo $display;?>;">
						<div class="mt5 payBox clearfix">
							<ul class="listArea">
							<?php
								foreach($pcode_list as $p_val){
								$name = $utility->remove_quoted($p_val['name']);	 // (쌍)따옴표 등록시 필터링
								$checked = ($search_sel_1[$val['code']]==$p_val['code']) ? "checked" : "";

								$name_val = "";
								if(stristr($name,'~')){	 // 10000 ~ 20000
									$names = explode('~',$name);
									$names_cnt = count($names);
									for($n=0;$n<$names_cnt;$n++){
										if(strlen(trim($names[$n])) >=5){
											$names[$n] = trim($names[$n]);
										}
									}
									$name_val = implode($names,'~');
								} else {
									$name_val = $name;
								}
								$name_val .= ($p_val['etc_0']) ? " ".$pay_level[$p_val['etc_0']] : " 원";
							?>
							<li id="category_middle_list_<?php echo $p_val['code'];?>">
								<span><!-- class="checkOn" -->
									<input type="radio" name="<?php echo $search_mode;?>[<?php echo $val['code'];?>]" id="<?php echo $search_mode;?>_1_<?php echo $p_val['code'];?>" value="<?php echo $p_val['code'];?>" onclick="category_middles_radio(this,'<?php echo $val['code'];?>');" <?php echo $checked;?>><label for="<?php echo $search_mode;?>_1_<?php echo $p_val['code'];?>"><?php echo $name_val;?></label>
								</span>
							</li>
		...
		<div id="JobListForm" class="positionR mt30 clearfix">
			...
			<div class="ListFunc positionA" style="top:-5px; right:0;">
				<span class="choiceLink pr5">
					<span class="Link1"><a href="./alba_list_pay.php?<?php echo $list_list['send_url'];?>&view_type=list#result"><img width="25" height="19" alt="" src="../images/icon/icon_choiceLink1_<?php echo (!$view_type||$view_type=='list')?'on':'off';?>.gif" class="vb" id="choiceLink_list"></a></span>
					<span class="Link2"><a href="./alba_list_pay.php?<?php echo $list_list['send_url'];?>&view_type=gallery#result"><img width="25" height="19" alt="" src="../images/icon/icon_choiceLink2_<?php echo ($view_type=='gallery')?'on':'off';?>.gif" class="vb" id="choiceLink_gallery"></a></span>
				</span>
				<span>
					<select title="pagesize" name="sort" style="width:100px;" class="ipSelect2" onchange="list_sorting(this,'alba_list_pay');">
