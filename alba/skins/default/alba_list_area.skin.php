...
<script type="text/javascript" src="<?php echo $alice['alba_path'];?>/skins/default/alba_list_area.skin.js"></script>
...
<section id="content" class="content_wrap clearfix">
	...
	<div class="content2_wrap clearfix"> 
		...
		<p> <a href="/">HOME</a> > <a href="<?php echo $alice['alba_path'];?>">채용정보</a> > <a href="<?php echo $alice['alba_path'];?>/alba_list_area.php">지역별 채용정보</a> > <strong>지역별 채용정보</strong> </p>
		</div>
		<?php /* //navigation */ ?>
			...
			<div class="jobContentWrap positionR mt10 clearfix">
				<div class="localWrap  clearfix">
					...
					<div id="category_second">
						...
						<?php 
						if($mode=='search' && $alba_area_1){ // 검색일때 
							$checked_arr = array();
							foreach($alba_area_1 as $area_key => $area_val){
							$pcode_list = $category_control->category_pcodeList('area',$area_key,"", " and `view` = 'yes' ");
							$display = ($area_key == $category_top_val[0]) ? 'block' : 'none';
						?>
							<div class="listLocal clearfix category_middle" id="category_middle_<?php echo $area_key;?>" style="display:<?php echo $display;?>;">
						...
						?>
									<li id="category_middle_list_<?php echo $val['code'];?>">
										<span>
											<input type="checkbox" name="<?php echo $search_mode;?>_1[<?php echo $val['p_code'];?>][]" id="<?php echo $search_mode;?>_1_<?php echo $val['code'];?>" class="<?php echo $search_mode;?>_1_<?php echo $val['p_code'];?> <?php echo $second_all;?>" value="<?php echo $val['code'];?>" onclick="category_middles(this,'<?php echo $val['p_code'];?>');" <?php echo $checked;?>/>
						...
						?>
								...
								if($category_middle_val) {
									...
									if(stristr($val,'all')) continue;
									$sub_list = $category_control->category_pcodeList('area',$val,"", " and `view` = 'yes' ");
									$display = (@in_array($val,$checked_arr)) ? 'block' : 'none';
								?>
									<div class="mt5 smallArea border_color1 bg_color2 clearfix category_sub category_sub_<?php echo $val;?>" id="category_sub_<?php echo $val;?>" style="display:<?php echo $display;?>;">
										<ul class="listArea">
										<?php
										...
										<li id="category_sub_list_<?php echo $val['code'];?>">
											<span>
												<input type="checkbox" name="<?php echo $mode;?>_2[<?php echo $middle_category['p_code'];?>][<?php echo $val['p_code'];?>][]" id="<?php echo $mode;?>_2_<?php echo $val['code'];?>" class="<?php echo $mode;?>_2_<?php echo $val['p_code'];?> <?php echo $third_all;?>" value="<?php echo $val['code'];?>" onclick="category_subs(this,'<?php echo $val['p_code'];?>','<?php echo $middle_category['p_code'];?>');" <?php echo $checked;?>/>
						...
						<div class="listLocal clearfix category_middle" id="category_middle_<?php echo $category_area[0]['code'];?>" style="display:;">
							<div class="mt5 mediumArea clearfix">
								<ul class="listArea">
								<?php
								$pcode_list = $category_control->category_pcodeList('area',$category_area[0]['code'],"", " and `view` = 'yes' ");
								foreach($pcode_list as $val){
								$name = $utility->remove_quoted($val['name']);	 // (쌍)따옴표 등록시 필터링
								$category_count = $alba_user_control->get_category_count_area($val['code']);
								if(stristr($val['code'],'all')){
								...
								<li id="category_middle_list_<?php echo $val['code'];?>">
									<span>
										<input type="checkbox" name="<?php echo $search_mode;?>_1[<?php echo $val['p_code'];?>][]" id="<?php echo $search_mode;?>_1_<?php echo $val['code'];?>" class="<?php echo $search_mode;?>_1_<?php echo $category_area[0]['code'];?> <?php echo $second_all;?>" value="<?php echo $val['code'];?>" onclick="category_middles(this,'<?php echo $category_area[0]['code'];?>');"/>
										<label for="<?php echo $search_mode;?>_1_<?php echo $val['code'];?>"><?php echo $name;?></label>
									</span>
									<?php if($is_all==false){ ?>
									<span class="sfont text_color1"> (<?php echo number_format($category_count);?>)</span>
							...
							<div id="category_third_<?php echo $category_area[0]['code'];?>"></div>
						</div>
				...
				<?php include_once "./views/_include/area_search.php";?>
			</div>
		</div>
		</form>
		<!--  // 시도별 검색영역 -->
		...
		<div id="JobListForm" class="positionR mt30 clearfix">
			...
			<div class="ListFunc positionA" style="top:-5px; right:0;">
				<span class="choiceLink pr5">
					<span class="Link1"><a href="./alba_list_area.php?<?php echo $list_list['send_url'];?>&view_type=list#result"><img width="25" height="19" alt="" src="../images/icon/icon_choiceLink1_<?php echo (!$view_type||$view_type=='list')?'on':'off';?>.gif" class="vb" id="choiceLink_list"></a></span>
					<span class="Link2"><a href="./alba_list_area.php?<?php echo $list_list['send_url'];?>&view_type=gallery#result"><img width="25" height="19" alt="" src="../images/icon/icon_choiceLink2_<?php echo ($view_type=='gallery')?'on':'off';?>.gif" class="vb" id="choiceLink_gallery"></a></span>
				</span>
				<span>
