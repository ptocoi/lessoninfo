/*
* /application/alba/skins/default/search.skin.js
* @author Harimao
* @since 2013/09/17
* @last update 2013/11/12
* @Module v3.5 ( Alice )
* @Brief :: Alba search Script
* @Comment :: 정규직 검색시 사용하는 공통 스크립트 / 함수
*/
$(function(){
...
var category_tops = function( code ){	// 1차 (최상위) 카테고리 선택
	...
	switch(search_mode){
		...
		case 'alba_type':		// 업직종별 검색
		case 'alba_area':		// 지역별 검색
		case 'alba_subway':	// 역세권 검색
		case 'alba_college':	// 대학가 검색
		case 'alba_pay':			// 급여별 검색
		case 'alba_pay':			// 급여별 검색
		case 'alba_list_adult':	// 성인분류
			...
			/*
			$.post('./views/_load/category.php', { mode:search_mode, type:'second', code:code }, function(result){
			});
			*/
		...
		case 'alba_term':	// 근무기간별 검색
		
			alert(type);

		break;

		case 'alba_target':	// 대상별 검색
...
var category_middles = function( vals, p_code, etc_0 ){	// 2차 카테고리 선택
	var code = vals.value;
...
var category_middles_radio = function( vals, p_code ){	// 2차 카테고리 선택 (라디오)
	var code = vals.value;
	var checked = vals.checked;
	...
	// 검색 조건
	var first_text = $('#category_top_'+p_code+' a').text();
	var second_text = $('#category_middle_list_'+code+' label').text();

	if(category_top_length){
		for(i=0;i<category_top_length;i++){
			if( category_top[i] == p_code){
				$('#select_'+p_code).remove();
			}
		}
	}

	var append_list = "<li id=\"select_"+p_code+"\" class=\"select_"+p_code+" category\">"+first_text+" > "+second_text+"&nbsp;</li>";
	// <button type=\"button\" class=\"close\" onclick=\"search_close('category','"+search_mode+"','"+p_code+"','"+code+"');\">x</button>
	$('.selectList').append(append_list);	// 1차 + 2차 카테고리 합쳐서 추가
	...
	$("#category_top_val").val(category_top);	// 1차 선택 카테고리 값 저장
	$("#category_middle_val").val(code);	// 2차 선택 카테고리 값 저장
...
var category_middle_all = function( vals, first_code, text ){
	var sel = vals.value;
	var checked = vals.checked;

	if(checked){
		$('.'+search_mode+'_sel_'+first_code).attr('checked',false);		// 체크 필드 초기화
		$('.'+search_mode+'_sel_'+first_code).attr('disabled',true);		// 체크 필드 사용제한
		$('.category_sub_'+first_code).hide();	// 3차 지역값 제거
		$('.select_'+first_code).remove();	 // 선택된 조건 지역값 제거
		$('.'+search_mode+"_sels_"+first_code).remove();

		// 선택된 조건 전체 추가
		$('.selectList').append("<input type=\"hidden\" name=\""+search_mode+"_sels[]\" value=\""+first_code+"/"+sel+"\" id=\""+search_mode+"_sels_"+first_code+"\" class=\""+search_mode+"_sels_"+first_code+"\" />");
	} else {
		$('.'+search_mode+'_sel_'+first_code).attr('disabled',false);
		$('.'+search_mode+'_sels_'+first_code).remove();
	}
}
var category_sub_all = function( vals, first_code, second_code, first_text, second_text ){
	var sel = vals.value;
	var checked = vals.checked;
	
	//alert(sel+" @ "+first_code+" @ "+second_code+" @ "+first_text+" @ "+second_text);
	// all @ 20130716174853_3466 @ 20130716174927_3418 @ 서울 @ 강남구
	// alba_area_sels_20130716174853_3466_20130729180856_2300

	if(checked){
		$('.'+search_mode+'_sel_'+second_code).attr('checked',false);		// 체크 필드 초기화
		$('.'+search_mode+'_sel_'+second_code).attr('disabled',true);		// 체크 필드 사용제한
		$('.'+search_mode+'_sels_'+first_code+'_'+second_code).remove();

		// 선택된 조건 전체 추가
		$('.selectList').append("<input type=\"hidden\" name=\""+search_mode+"_sels[]\" value=\""+first_code+"/"+second_code+"\" id=\""+search_mode+"_sels_"+first_code+"_"+second_code+"\" class=\""+search_mode+"_sels_"+first_code+"_"+second_code+"\" />");

	} else {
		$('.'+search_mode+'_sel_'+second_code).attr('disabled',false);
		$('.'+search_mode+'_sels_'+first_code+'_'+second_code).remove();
	}
}
...
var area_thirds = function( vals, p_code, first_code ){	// 3차 지역 검색 선택 third code, second code, first code
	...
	if( checked == true ){	// 체크했다면
		...
		$('.selectList').append("<input type=\"hidden\" name=\"area_sels[]\" value=\""+first_code+"/"+p_code+"/"+code+"\" id=\"area_sels_"+first_code+"_"+p_code+"_"+code+"\" class=\"area_sel_"+first_code+"\" />");
		...
		if(checked_length == 0){
			...
			$('.selectList').append(append_list);	// 1차 + 2차 지역값을 합쳐서 추가
			$('#area_sels_'+first_code+"_"+p_code+"_"+code).remove();
			$('.selectList').append("<input type=\"hidden\" name=\"area_sels[]\" value=\""+first_code+"/"+p_code+"\" id=\"area_sels_"+first_code+"_"+p_code+"\" class=\"area_sel_"+first_code+"\" />");
		}
...
var searchFrm_reset = function(){	// 초기화
	...
	var go_clearUrl = location.href;
	var go_clearText = go_clearUrl.split('?');
	var go_cleardomain = go_clearUrl.indexOf('type=day');
	
	if(go_cleardomain == -1) {
		location.href = go_clearText[0];
    }else{
		location.href = go_clearText[0] + "?type=day";
    }
	/*
	...
	$("input[name='wr_date']").attr('checked',false);
	$("input[name='wr_week']").attr('checked',false);
	$("input[name='wr_pay_support']").attr('checked',false);
	$("input[name='wr_target']").attr('checked',false);
