<?php
		/*
		* /application/alba/alba_list_area.php
		* @author Harimao
		* @since 2013/08/12
		...
		*/
		...
		$search_mode_sels = $_GET['alba_area_sels'];
		$search_sel_1 = $_GET['alba_area_1'];
		$search_sel_2 = $_GET['alba_area_2'];
		...
		if($area){
			$list_con .= " and ( INSTR( `wr_area_0`, '".$area."' ) or INSTR( `wr_area_1`, '".$area."' ) or INSTR( `wr_area_2`, '".$area."' ) )";
		}
		...
		$pages = $utility->user_get_paging($page_rows, $page, $total_page,"./alba_list_area.php?page_rows=".$page_rows."&".$list_list['send_url']."&page=");

		##
		# Include view
		if(is_file($alice['self'] . 'views/alba_list_area.html'))
			include_once $alice['self'] . 'views/alba_list_area.html';
		else
			$config->error_info( $alice['self'] . 'views/alba_list_area.html' );
