<?php
		/*
		* /application/alba/alba_list_term.php
		* @author Harimao
		* @since 2013/08/12
		* @last update 2015/03/17
		* @Module v3.5 ( Alice )
		* @Brief :: Alba list
		* @Comment :: 근무기간.요일.시간별 정규직 리스팅
		*/
		...
		if($type=='short'){	 // 단기 정규직정보
			$etc_0 = $category_control->category_codeLists("alba_date"," and `etc_0` = '1' ");
			$etc_0_cnt = count($etc_0);
			$etc_0_val = array();
			for($i=0;$i<$etc_0_cnt;$i++){
				array_push($etc_0_val,"'".$etc_0[$i]['code']."'");
			}
			//  and `wr_report` = 0
			$list_con = " where `wr_open` = 1 and `wr_is_adult` = 0 and `is_delete` = 0 ";			
			$list_con .= " and `wr_date` in (".implode($etc_0_val,",").") ";
		} else if($type=='long'){	 // 장기 정규직정보
			$etc_1 = $category_control->category_codeLists("alba_date"," and `etc_1` = '1' ");
			$etc_1_cnt = count($etc_1);
			$etc_1_val = array();
			for($i=0;$i<$etc_1_cnt;$i++){
				array_push($etc_1_val,"'".$etc_1[$i]['code']."'");
			}
			//  and `wr_report` = 0
			$list_con = " where `wr_open` = 1 and `wr_is_adult` = 0 and `is_delete` = 0 ";			
			$list_con .= " and `wr_date` in (".implode($etc_1_val,",").") ";
		} else {
			//  and `wr_report` = 0
			$list_con = " where `wr_open` = 1 and `wr_is_adult` = 0 and `is_delete` = 0 ";
		}

		$list_con .= " and (`wr_volume_date` >= curdate() or `wr_volume_always` = 1 or `wr_volume_end` = 1 ) ";
		...
		$pages = $utility->user_get_paging($page_rows, $page, $total_page,"./alba_list_term.php?page_rows=".$page_rows."&".$list_list['send_url']."&page=");

		##
		# Include view
		if(is_file($alice['self'] . 'views/alba_list_term.html'))
			include_once $alice['self'] . 'views/alba_list_term.html';
		else
			$config->error_info( $alice['self'] . 'views/alba_list_term.html' );
