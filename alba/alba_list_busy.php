<?php
		/*
		* /application/alba/alba_list_busy.php
		* @author Harimao
		* @since 2013/08/12
		...
		*/
		...
		//  and `wr_report` = 0
		$list_con .= " where `wr_open` = 1 and `wr_is_adult` = 0 and `wr_service_busy` >= curdate() and `is_delete` = 0 and (`wr_volume_date` >= curdate() or `wr_volume_always` = 1 or `wr_volume_end` = 1 ) ";
		...
		$pages = $utility->user_get_paging($page_rows, $page, $total_page,"./alba_list_busy.php?page_rows=".$page_rows."&".$list_list['send_url']."&page=");

		##
		# Include view
		if(is_file($alice['self'] . 'views/alba_list_busy.html'))
			include_once $alice['self'] . 'views/alba_list_busy.html';
		else
			$config->error_info( $alice['self'] . 'views/alba_list_busy.html' );
