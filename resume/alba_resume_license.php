<?php
		/*
		* /application/resume/alba_resume_license.php
		* @author Harimao
		* @since 2013/08/13
		...
		*/
		...
		if($mode=='search'){	 // 알바 검색
			...
			*/
			...
			$search_license = urldecode(trim($search_license));	// 검색어(자격증명)
		...
		$pages = $utility->user_get_paging($page_rows, $page, $total_page,"./alba_resume_license.php?page_rows=".$page_rows."&".$list_list['send_url']."&page=");

		##
		# Include view
		if(is_file($alice['self'] . 'views/alba_resume_license.html'))
			include_once $alice['self'] . 'views/alba_resume_license.html';
		else
			$config->error_info( $alice['self'] . 'views/alba_resume_license.html' );
