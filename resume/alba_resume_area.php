<?php
		/*
		* /application/resume/alba_resume_area.php
		* @author Harimao
		* @since 2013/08/13
		...
		*/
		...
		if($mode=='search'){	 // 이력서 검색
			...
			*/
			...
			$alba_area_1 = $_GET['alba_area_1'];	// 2차 지역 배열
			$alba_area_2 = $_GET['alba_area_2'];	// 3차 지역 배열

			$area_sels = $_GET['resume_area_sels'];	// 지역 검색 값
			...
			if($area){
				$mode = "search";
				$category_top_val[0] = $area;
				$alba_area_1[$area] = array( );
			}
		}

		$search_mode_sels = $_GET['resume_area_sels'];
		$search_sel_1 = $_GET['resume_area_1'];
		$search_sel_2 = $_GET['resume_area_2'];
		...
		if($area){
			$list_con .= " and ( `wr_area0` = '".$area."' or `wr_area1` = '".$area."' or `wr_area2` = '".$area."' )";
		}
		...
		$pages = $utility->user_get_paging($page_rows, $page, $total_page,"./alba_resume_area.php?page_rows=".$page_rows."&".$list_list['send_url']."&page=");

		##
		# Include view
		if(is_file($alice['self'] . 'views/alba_resume_area.html'))
			include_once $alice['self'] . 'views/alba_resume_area.html';
		else
			$config->error_info( $alice['self'] . 'views/alba_resume_area.html' );
