<?php if (!defined("_ALICE_")) exit; // 개별 페이지 접근 불가 
	...
	if($use_print==1){ 
?>
	<div class="mt30 comImages list2 positionR clearfix">
		...
		<div class="imageType positionR clearfix">
			<ul class="clearfix">
			<?php 
				for($i=0;$i<$rows;$i++){ 
			...
			?>
			<li>
				<a class="img" href="./board.php?board_code=<?php echo $board_code;?>&code=<?php echo $list[$i]['code'];?>&bo_table=<?php echo $board['bo_table'];?>&wr_no=<?php echo $list[$i]['wr_no'];?>"><img class="vm" src="<?php echo $thumb_image;?>" alt="" style="width:<?php echo $width;?>px;height:<?php echo $height;?>px;"></a>
				<a class="mt5 title" href="./board.php?board_code=<?php echo $board_code;?>&code=<?php echo $list[$i]['code'];?>&bo_table=<?php echo $board['bo_table'];?>&wr_no=<?php echo $list[$i]['wr_no'];?>"><strong><?php echo $list[$i]['subject'];?></strong></a>
				<span class="mt3"><img class="vm mr5" width="12" height="12"  src="../images/icon/ico_time1.gif" alt="time"><?php echo $wr_datetime;?></span>
			</li>
			<?php } // for end. ?>
			</ul>
		</div>
...
<?php } else if($use_print==2){ ?>
	...
	<div class="mt30 comImages list1 positionR clearfix">
		<h3><img class="mr5 vm bg_color4" width="13" height="13"  src="../images/icon/icon_arrow_4.png" alt=""><?php echo stripslashes($board['bo_subject']);?></h3>
		<em class="positionA" style="top:10px; right:10px;"><a href="./board.php?board_code=<?php echo $board_code;?>&code=<?php echo $board['code'];?>&bo_table=<?php echo $board['bo_table'];?>"><img width="42" height="15" class="vb bg_color4" src="../images/icon/icon_more1.gif" alt=""></a></em>
		<div class="imageType positionR clearfix">
		<ul class="clearfix">
		<?php 
			for($i=0;$i<$rows;$i++){ 
			preg_match("/<img[^>]*src=[\"']?([^>\"']+)[\"']?[^>]*>/i", $list[$i]['content'], $content_image);	// 내용상 이미지가 존재하는 경우
			if($content_image){
				$image_pathinfo = pathinfo($content_image[1]);	// 이미지 경로 확인
				if(stristr($image_pathinfo['dirname'],$_SERVER['HTTP_HOST'])){	// 현 서버에 있는 이미지라면
					$dir_name = $image_pathinfo['dirname'];
