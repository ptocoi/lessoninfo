...
var mbNICK_duplicateCheck = function( mb_id ){	// 닉네임 중복 확인
	var mb_nick = $('#mb_nick').val();
	$.post('./views/_ajax/member.php', { mode:'mb_nick_duplicate', mb_nick:mb_nick, mb_id:mb_id }, function(result){
		if(!mb_nick || mb_nick==''){
			alert("닉네임을 입력해 주세요.");
			$('#mb_nick').focus();
			return false;
...
var photo_options = "";
var update_photos = function( mode, no ){	// 회사이미지 등록/삭제
	if(mode=='update'){	// 등록
		...
		if(confirm('회사이미지를 삭제하시겠습니까?\n\n한번 삭제된 데이터는 복구가 불가능합니다.')){
			$.post('../member/process/regist.php', { mode:'photo_delete', mb_id:mb_id, photo_no:no }, function(result){
...
var photos_submit = function(){	// 회사이미지 등록하기 버튼 클릭시
	...
	$('#MemberUpdateFrm #mode').val('photo_upload');	// mode 변경
...
var photoResponse = function(responseText, statusText, xhr, $form){	// 회사이미지 전송 후
	$('#MemberUpdateFrm #mode').val('member_update');	// mode 변경
