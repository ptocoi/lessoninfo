...
var pay_submit = function(){	// 선택 서비스 정보 form ajax 전송
	...
	*/
	...
	if(total_price == 0){	// 무료일땐 무통장으로 처리
	...
	$('#ServicePayment').attr("action", "./process/sms_pay.php");
	...
	if(responseText == '0006' ){
		...
		if(pay_method=='bank'){	// 무통장 입금
			...
			if(validate(PayGateFrm)){
				if(mb_type=='company'){
					action = "../company/process/alba_pay_bank.php"
				} else if(mb_type=='individual'){
					action = "../individual/process/alba_resume_bank.php"
				}
				$('#PayGateFrm').attr("action",action);
		...
		if(price=='0'){	// 무료일때
			location.href = "../"+mb_type+"/success.php?oid="+oid;
			return false;
		}
