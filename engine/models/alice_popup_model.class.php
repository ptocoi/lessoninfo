								...
								if(($val['popup_view'] && $val['popup_begin'] <= $alice['time_ymdhis'] && $val['popup_end'] >= $alice['time_ymdhis']) || $val['popup_unlimit']){	// 출력 확인
									...
									if($val['popup_type']){	// 레이어
										...
										if($val['popup_title_view'])	// 제목 출력 유무
											$result .= "<tr><td style='height:25px;text-align:left;padding-left:5px;cursor:move;color:#".$subject_color."' class='popupLayer' id='popupHandle_".$no."'>".stripslashes($val['popup_title'])."</td></tr>";
										$result .= "<tr>";
										$result .= "<td class=\"pcontents\" style=\"".$background.";\">".nl2br(stripslashes($val['popup_content']))."</td>";
										$result .= "</tr>";
										if($skin['cookie_time']){
											$result .= "<tr>";
											...
											$result .= "<label class='hand' style='color:#".$font_color."'><input name=\"popupCloseNo[]\" type=\"checkbox\" value=\"".$no."\" class=\"check\" id='popupClose_".$no."'>".strtr($val['cookie_time'],$popup_control->cookie_arr)."하루동안 열지 않기</label>";
											$result .= "<span class=\"bar\"><a onclick=\"popup_Close('".$no."');\">close<b class=\"pl2\">x</b></a></span>";
											$result .= "</dl>";
											$result .= "</td>";
											$result .= "</tr>";
										}
