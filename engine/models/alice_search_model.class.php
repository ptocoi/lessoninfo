			...
			var $fail_code = array(
				...
				function __Searching( $page="", $page_rows="" ){
					...
					global $service_control;
						...
						if( $service_check['is_pay'] ){
							$alba_resume_con .= " and ( `wr_service_main_focus` >= curdate() or `wr_service_sub_focus` >= curdate() or `wr_service_basic` >= curdate() and `wr_service_basic_sub` >= curdate() ) ";
						}
				...
				function get_search( $limit=50 ){
						...
						$query = " select * from `".$this->search_table."` where `wr_udate` like '".date('Y-m-d')."%' order by `wr_hit` desc limit " . $limit;
						$result['today'] = $this->query_fetch_rows($query);

						$query = " select * from `".$this->search_table."` where `wr_wdate` between date_sub(curdate(), interval -1 day) and curdate() order by `wr_hit` desc limit " . $limit;
						$result['yesterday'] = $this->query_fetch_rows($query);

						$query = " select * from `".$this->search_table."` where `wr_wdate` between date_sub(curdate(), interval 1 month) and curdate() order by `wr_hit` desc limit " . $limit;
