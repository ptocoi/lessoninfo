<?php
		...
		*/
		class alice_alba_user_model extends DBConnection {
			...
			//var $pay_conference = array( 0 => "", 1 => "추후협의", 2 => "면접후결정" );
						...
						switch($service){
							...
							case 'adult_platinum':
								...
								if($list['input_type']=='self'){	// 관리자 직접 입력
									...
									//$logo_path = $alice['data_member_path'] . "/" . $wr_id . "/logo/" . $company_info['mb_logo'];
									$logo_path = $alice['data_alba_path'] . "/" . $list['etc_0'];
									//if(file_exists($logo_path."/".$list['etc_0'])){
									if( file_exists($logo_path) && is_file($logo_path) ){
										if($is_service['service_platinum_main_logo']){
											//$logo = $this->logo_print( $logo_path."/".$list['etc_0'], $get_payment['pay_alba_platinum_logo_effect'], $list['company_name'] );
											$logo = $this->logo_print( $logo_path, $get_payment['pay_main_platinum_logo_effect'], $list['company_name'] );
											$result['wr_logo'] = $logo;
										} else if($is_service['service_platinum_sub_logo']){
											//$logo = $this->logo_print( $logo_path."/".$list['etc_0'], $get_payment['pay_alba_platinum_logo_effect'], $list['company_name'] );
											$logo = $this->logo_print( $logo_path, $get_payment['pay_main_platinum_logo_effect'], $list['company_name'] );
											$result['wr_logo'] = $logo;
										} else {
									...
									if($is_service['service_platinum_main_logo']){
										...
										$logo = $this->logo_print( $wr_logo, $get_payment['pay_alba_platinum_logo_effect'], $list['company_name'] );
										$result['wr_logo'] = $logo;
									} else {
										$result['wr_logo'] = "<img alt=\"".$list['company_name']."\" src=\"".$wr_logo."\" title=\"".$list['company_name']." style=\"max-width:100px; max-height:50px;\"\"/>";
									}
				...
				*/
				...
				function get_category_count( $code, $search_field ){
					...
					global $service_control;
						...
						$query .= " and `wr_open` = 1 and `wr_report` = 0 and `wr_is_adult` = 0 and `is_delete` = 0 ";
					...
					global $logo_control;
						...
						if($company){
							...
							if($company['mb_logo_sel']){
								...
								$result = (is_file($company_logo_file)) ? $company_logo_file : "../images/basic/bg_noLogo.gif";	 // 업소회원 로고 사진

							} else {

								$company_logo_file = $alice['data_member_path']."/".$company['mb_id']."/logo/".$company['mb_logo'];
								$result = (is_file($company_logo_file)) ? $company_logo_file : "../images/basic/bg_noLogo.gif";	 // 업소회원 로고 사진
							...
							if( is_file($alice['data_alba_path']."/".$alba['etc_0']) ){
								...
								$result = "../images/basic/bg_noLogo.gif";
							}
