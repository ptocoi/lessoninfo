		...
		class alice_alba_model extends DBConnection {
			...
			var $fail_code = array(
					...
					global $utility, $config;
						...
						if($mode=='search'){
							...
							if($wr_service){
								...
								$service_arr = array( 
									"platinum" => " `wr_service_platinum` >= now() or `wr_service_platinum_sub` >= now() ",
									"prime" => " `wr_service_prime` >= now() ",
									"grand" => " `wr_service_grand` >= now() ",
									"banner" => " `wr_service_banner` >= now() or `wr_service_banner_sub` >= now() ",
									"busy" => " `wr_service_busy` >= now() ",
									"list" => " `wr_service_list` >= now() or `wr_service_list_sub` >= now() ",
									"basic" => " `wr_service_basic` >= now() or `wr_service_basic_sub` >= now() ",
									"logo" => " `wr_service_platinum_main_logo` >= now() or `wr_service_platinum_sub_logo` >= now() or `wr_service_prime_main_logo` >= now() or `wr_service_grand_main_logo` >= now() ",
									"neon" => " `wr_service_neon` >= now() ",
									"icon" => " `wr_service_icon` >= now() ",
									"blink" => " `wr_service_blink` >= now() ",
									"jump" => " `wr_service_jump` >= now() ",
								);
				...
				function get_alba_for_id( $wr_id ){
						...
						$query = " select * from `".$this->alba_table."` where `wr_id` = '".$wr_id."' and `wr_open` = 1 and `wr_report` = 0 order by `no` desc ";
