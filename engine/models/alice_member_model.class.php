					...
					global $utility, $config;
						...
						if($sort){
							if($sort == 'mb_service_open') // 열람기간은 서비스 테이블에서
								$order = " group by c.".$sort." ";
							else if($sort=='mb_company_name')	// 기업회원명은 기업회원 테이블에서
								$order = " group by b.".$sort." ";
							else
								$order = " group by a.".$sort." ";
						}
						...
						if($mode=='search'){
							...
							if($search_field==''){	// 통합검색 일때
								...
								if(preg_match("/[a-zA-Z]/", $search_keyword)) {
									...
									$search_que .= " INSTR(LOWER(".$member_email."), LOWER('".$search_keyword."')) ";
									...
									$search_que .= " INSTR(".$member_email.", '".$search_keyword."') ";
