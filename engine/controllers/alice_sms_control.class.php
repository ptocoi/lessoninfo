				...
				*/
				function send_sms_Members( $msg, $rphone_list, $sphone ){
						...
						for($i=0;$i<$rphone_list_cnt;$i++){
							...
							$result = $this->netfu_sms_Send( $msg, $rphone_list_exp[0], $sphone, $destination );
					...
					global $admin_control;
						...
						if($msg_use){	// 사용한다면
							...
							if(@in_array('sms',$mb_receive)){	 // 문자 수신 확인
								$result = $this->netfu_sms_Send( $msg_replace['msg'], $member['mb_hphone'], $env['call_center'], $destination, $member );	// 우선 회원한테 발송
							}
							...
							if($msg_config['msg_is_admin']){	// 관리자에게도 발송한다면
								...
								$result = $this->netfu_sms_Send( $msg_replace['admin_msg'], $sms_config['sms_admin_num'], $env['call_center'], $destination, $datas );
