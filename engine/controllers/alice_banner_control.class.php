					...
					global $alice, $utility;
						...
						if($type=='image'){	// 이미지
							...
							if($val['file_type'] > 3){	// swf 파일
								...
								$result = ($val['url']) ? 
									"<a href='".$utility->set_http($val['url'])."' ".$target." ".$class." id=\"".$val['no']."\"><img src='" . $alice['data_banner_path'] . "/" . $content . "' width='".$val['width']."' height='".$val['height']."'/></a>" :
									"<img src='" . $alice['data_banner_path'] . "/" . $content . "' width='".$val['width']."' height='".$val['height']."'/>";
