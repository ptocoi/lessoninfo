		...
		class alice_mail_control extends alice_mail_model {
				...
				function mail_replaces( $mb_email, $mail_content, $mail_skin="", $cs_no="", $employ_no="", $resume_no="", $alba_no="", $alba_resume_no="", $wr_id="", $proposal_no="" ){
					...
					global $alba_file_control;
						...
						$mb = $member_control->get_memberEmail($mb_email);
						$mb_name = $mb['mb_name'];
						$mb_email = $mb['mb_email'];
						$mb_id = $mb['mb_id'];
						$mb_wdate = $mb['mb_wdate'];
						...
						*/
						...
						if($alba_resume_no){	 // 알바 이력서
							...
							$become_resume = "<a href='".$alice['url']."/resume/alba_resume_detail.php?no=".$alba_resume_no."' target='_blank'>".stripslashes($get_alba_resume['wr_subject'])."</a>";
						} else {	 // 회사 자사양식
							$get_member = $member_control->get_member($wr_id);	// 회원 정보
						...
						if($receive_info['etc_2']){
							...
							$become_form = "<a href='".$alice['url'] . "/data/email/" . $receive_info['etc_2']."' target='_blank'>".$get_member['mb_name']."_이력서.".$file_ext."</a>";
						} else {
							$become_form = "첨부된 지원양식이 없습니다.";
						}
						$receive_info_etc_3 = explode("/",$receive_info['etc_3']);	 // 첨부파일
						if($receive_info['etc_3']){
							$become_file = "<a href='".$alice['url'] . "/data/email/" . $receive_info['etc_3']."' target='_blank'>이력서첨부파일</a>";
						} else {
						...
						if($receive_info['etc_5']){
							...
							for($i=0;$i<$receive_info_etc_5_cnt;$i++){
								...
								$receive_info_etc_5_arr[$i] = "<a href='".$alice['url'] . "/data/alba/" . $get_file['wr_content']."' target='_blank'>".$get_file['wr_source']."</a>";
							}
