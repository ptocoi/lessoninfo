...
<script type="text/javascript" src="<?php echo $alice['individual_path'];?>/skins/default/open_pay.skin.js"></script>
...
<section id="content" class="content_wrap clearfix">
	...
	<div class="NowLocation mt35 clearfix">
		<p> <a href="/">HOME</a> > <a href="<?php echo $alice['individual_path'];?>/">기업서비스</a> > <strong>결제하기</strong> </p>
	</div>
	<?php /* //navigation */ ?>
	...
	<div class="listWrap positionR mt20">
		...
		<input type="hidden" name="pay_type" value="etc_alba" id="pay_type"/>
			...
			<div id="listForm" class="mainTopBorder positionR mt30 clearfix">
				...
				<div class="customList1 mb10">
					...
					<?php if($etc_alba_date){ // 채용공고 열람 서비스 ?>
					<input type="hidden" name="<?php echo $sel_service;?>_date" value="<?php echo $_POST[$sel_service."_date"];?>"/>
					<tr id="etc_open">
						<td class="tc"  height="50"><span><strong>채용공고 열람 서비스</strong></span></td>
						<td class="tl">
							<span>
							<?php 
							if($etc_alba_dates[1]=='count') {	// 건별 결제
								echo "<span class='b'>".$etc_alba_dates[0].$service_control->_unit($etc_alba_dates[1])."</span>";
								echo ($etc_alba_dates[5]) ? " (".$etc_alba_dates[5].$service_control->_unit($etc_alba_dates[6])." 이후 자동 소멸)" : "";
								...
								echo "오늘 + ".$etc_alba_dates[0].$service_control->_unit($etc_alba_dates[1]);
								echo "(".date('Y.m.d')."~".date('Y.m.d',strtotime("+".$etc_alba_dates[0]." ".$etc_alba_dates[1])).")";
								if($is_open_count){
						...
						<td class="tc">
							<span id="etc_open_price_val">
								...
								<input type="hidden" name="etc_open_price" id="etc_open_price" value="<?php echo $etc_alba_date;?>"/>
							</span>
						</td>
						<td class="tc"><span class="text_red"><strong id="etc_open_total_val">\<?php echo number_format($etc_alba_dates[4]);?></strong></span></td>
					</tr>
					<?php } ?>
					</tbody>
					</table>
